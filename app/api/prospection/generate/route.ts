...
export async function POST(req: NextRequest) {
  ...
  const systemPrompt = [
    ...
    "LANGUE : détecte la langue du PROSPECT à partir, dans l'ordre, du PROFIL LINKEDIN ENRICHI / FICHE LINKEDIN ENTREPRISE / CONTEXTE ENTREPRISE (pays, langue des contenus), puis du poste du prospect. Les INSTRUCTIONS UTILISATEUR ne définissent PAS la langue : un prospect anglophone reçoit un email en anglais même si les instructions sont en français (sauf si l'utilisateur demande explicitement une langue). En cas de doute, repli sur l'anglais. Rédige le subject ET le body dans cette langue (idiomes, ponctuation, signature inclus).",
